...
SHELL = /bin/sh
LDFLAGS = "-s -w -X tinifier/version.version=$(shell git rev-parse HEAD)"
...
.PHONY : help \
         ...
         clean
.DEFAULT_GOAL : help
.SILENT : require_docker require_dc lint gotest

help: ## Show this help
	@printf "\033[33m%s:\033[0m\n" 'Available commands'
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  \033[32m%-11s\033[0m %s\n", $$1, $$2}' $(MAKEFILE_LIST)

require_docker: # Make check - docker is installed?
	test -x $(DOCKER_BIN) || ( echo 'Installed `docker` is required. Install it `curl -SsL get.docker.com | sudo sh`'; exit 1 )

require_dc: # Make check - docker-compose is installed?
	test -x $(DC_BIN) || ( echo 'Installed `docker-compose` is required. Install it <https://docs.docker.com/compose/install/#install-compose>'; exit 1 )

image: require_docker ## Build docker image with app
...
build: require_dc ## Build app binary file
	$(DC_BIN) run $(DC_RUN_ARGS) app go build -ldflags=$(LDFLAGS) .

fmt: require_dc ## Run source code formatter tools
...
cover: require_dc ## Run app tests with coverage report
...
shell: require_dc ## Start shell into container with golang
	$(DC_BIN) run $(DC_RUN_ARGS) app bash

clean: require_docker require_docker ## Make clean
	$(DC_BIN) down -v -t 1
